...
app.post('/api/upload', upload.single('file'), (req, res) => {
  ...
  app.get('/api/lesson/today', (req, res) => {
    try {
      const dateStr = today();
      const total = db.prepare('SELECT COUNT(*) as c FROM daily_lessons').get().c;
      if (total === 0) return res.json({ lesson: null, date: dateStr });

      // 计算已完成的课数
      const completedCount = db.prepare('SELECT COUNT(DISTINCT lesson_id) as c FROM lesson_checkins WHERE completed = 1').get().c;

      // 查找今天是否已有分配的课
      const todayCheckin = db.prepare('SELECT * FROM lesson_checkins WHERE date = ?').get(dateStr);

      let lessonId;
      if (todayCheckin) {
        lessonId = todayCheckin.lesson_id;
      } else {
        // 递补机制：查找之前未完成的课（漏掉的日期）
        const missedCheckin = db.prepare(`
          SELECT l.id as lesson_id FROM daily_lessons l
          WHERE l.id NOT IN (SELECT lesson_id FROM lesson_checkins WHERE completed = 1)
          AND l.id NOT IN (SELECT lesson_id FROM lesson_checkins WHERE date = ?)
          ORDER BY l.id ASC LIMIT 1
        `).get(dateStr);

        if (missedCheckin) {
          // 有未完成的课，递补进来
          lessonId = missedCheckin.lesson_id;
        } else {
          // 所有课都完成了，按顺序循环
          lessonId = (completedCount % total) + 1;
        }

        // 创建今日记录
        db.prepare('INSERT OR IGNORE INTO lesson_checkins (lesson_id, date, completed) VALUES (?, ?, 0)').run(lessonId, dateStr);
      }

      const lesson = db.prepare('SELECT * FROM daily_lessons WHERE id = ?').get(lessonId);
      const checkin = db.prepare('SELECT * FROM lesson_checkins WHERE lesson_id = ? AND date = ?').get(lessonId, dateStr);
      ...
      console.error('/api/lesson/today error:', e);
      res.status(500).json({ error: e.message });
    }
  });

  // 打卡完成今日一课
  app.post('/api/lesson/checkin', (req, res) => {
    try {
      const { lesson_id, completed } = req.body;
      const dateStr = today();
      const exists = db.prepare('SELECT id FROM lesson_checkins WHERE lesson_id = ? AND date = ?').get(lesson_id, dateStr);
      if (exists) {
        db.prepare('UPDATE lesson_checkins SET completed = ? WHERE lesson_id = ? AND date = ?').run(completed ? 1 : 0, lesson_id, dateStr);
      } else {
        db.prepare('INSERT INTO lesson_checkins (lesson_id, date, completed) VALUES (?, ?, ?)').run(lesson_id, dateStr, completed ? 1 : 0);
      }
      // 统计
      const completedCount = db.prepare('SELECT COUNT(DISTINCT lesson_id) as c FROM lesson_checkins WHERE completed = 1').get().c;
      const total = db.prepare('SELECT COUNT(*) as c FROM daily_lessons').get().c;
      res.json({ ok: true, completedTotal: completedCount, totalLessons: total });
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
  });
  ...
  // 获取历史记录（最近N天）
  app.get('/api/lesson/history', (req, res) => {
    try {
      const days = parseInt(req.query.days) || 30;
      const checkins = db.prepare(`
        SELECT lc.*, dl.challenge, dl.category, dl.theme, dl.reason
        FROM lesson_checkins lc
        JOIN daily_lessons dl ON lc.lesson_id = dl.id
  ...
  // 获取统计
  app.get('/api/lesson/stats', (req, res) => {
    try {
      const total = db.prepare('SELECT COUNT(*) as c FROM daily_lessons').get().c;
      const completedCount = db.prepare('SELECT COUNT(DISTINCT lesson_id) as c FROM lesson_checkins WHERE completed = 1').get().c;
      const categoryStats = db.prepare(`
        SELECT dl.category, COUNT(DISTINCT lc.lesson_id) as done
        FROM daily_lessons dl
        LEFT JOIN lesson_checkins lc ON lc.lesson_id = dl.id AND lc.completed = 1
        GROUP BY dl.category
      `).all();
      // 连续天数
      const streakRow = db.prepare(`
        SELECT COUNT(DISTINCT date) as streak FROM lesson_checkins
        WHERE completed = 1 AND date >= date('now', 'localtime', '-30 days')
      `).get();
      res.json({
        total,
  ...
  // ===== 五年级语文上册教材 API =====
  const grade5ChinesePath = path.join(__dirname, 'data', 'grade5-chinese-vol1.json');
  const grade5ShengziPath = path.join(__dirname, 'data', 'grade5-chinese-shengzi.json');
  const grade5XieziPath = path.join(__dirname, 'data', 'grade5-chinese-xiezi.json');
  const grade5CiyuPath = path.join(__dirname, 'data', 'grade5-chinese-ciyu.json');

  app.get('/api/chinese/grade5/overview', (req, res) => {
    try {
      if (!fs.existsSync(grade5ChinesePath)) return res.status(404).json({ error: '教材数据未找到' });
      res.json(JSON.parse(fs.readFileSync(grade5ChinesePath, 'utf8')));
    } catch (e) { res.status(500).json({ error: e.message }); }
  });

  app.get('/api/chinese/grade5/shengzi', (req, res) => {
    try {
      if (!fs.existsSync(grade5ShengziPath)) return res.status(404).json({ error: '生字数据未找到' });
      res.json(JSON.parse(fs.readFileSync(grade5ShengziPath, 'utf8')));
    } catch (e) { res.status(500).json({ error: e.message }); }
  });

  app.get('/api/chinese/grade5/xiezi', (req, res) => {
    try {
      if (!fs.existsSync(grade5XieziPath)) return res.status(404).json({ error: '写字数据未找到' });
      res.json(JSON.parse(fs.readFileSync(grade5XieziPath, 'utf8')));
    } catch (e) { res.status(500).json({ error: e.message }); }
  });

  app.get('/api/chinese/grade5/ciyu', (req, res) => {
    try {
      if (!fs.existsSync(grade5CiyuPath)) return res.status(404).json({ error: '词语数据未找到' });
      res.json(JSON.parse(fs.readFileSync(grade5CiyuPath, 'utf8')));
    } catch (e) { res.status(500).json({ error: e.message }); }
  });
